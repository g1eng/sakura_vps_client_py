...
class ApiKey(BaseModel):
    ...
    role: StrictInt = Field(description="ロールID")
    token: StrictStr = Field(description="APIのアクセストークン。APIキー作成時とトークンローテーション時以外は情報を`*`でマスクして返します。")
    __properties: ClassVar[List[str]] = ["id", "name", "role", "token"]
